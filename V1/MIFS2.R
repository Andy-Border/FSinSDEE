MIFS <- function(dData,# data set
                 rFNum,# responce feature's column number
                 k = 5,# number of features selected from n features
                 Be= 0.75,
                 MIMatrix = NULL) {
  # Example:
  # fSet<- mifs(dData = ISBSG, rFNum = responseVNum,k=10);print(colnames(dData)[fSet])
  ...
  S <- c(S,firstFeature)
  mifs <- c(mifs,MIMatrix[rFNum,firstFeature])
  ...
  for (i in 1:(k-1)) { # select one feature at a time for k times
    #Calculate mifs Matrix
    tempMIFS <- c(rep(-10, ncol(dData))) 
    # mifs initialization -1 for default, the value will range 
    # from 0 to 1
    for (j in 1:ncol(dData)) {
      if (!isSelected[j]) { # calculate the mifs of remain features
        # mifs = I(C;fi)- (Beta(sum(I(fi,fs)))) Beta=1/|S|
        ICFI <- MIMatrix[j, rFNum]
        sumIofOtherFeature <- sum(MIMatrix[j, isSelected])
        tempMIFS[j] <- ICFI - Be * sumIofOtherFeature
      }
    }
    # select the max mifs feature
    maxmifs <- which.max(tempMIFS)
    isSelected[maxmifs] <- TRUE;
    S <- c(S,maxmifs);
    vmi <- c(vmi,MIMatrix[rFNum,maxmifs])
    mifs <- c(mifs,tempMIFS[maxmifs])
  }
  featureNames <- factor(colnames(dData)[S],levels=unique(colnames(dData)[S]))
  result <- data.frame(featureNames =featureNames,S=S,mifs=mifs,vmi=vmi)
  return(result)
}
